...
\begin{document}
  ...
  \begin{frame}
    ...
    A derivation in $T$
    \[ \Gamma \mid \varphi \vdash_T \psi \]
    is what we can derive 
    with the rules of inference 
    together with an axiom for each sequent of the theory.
  \end{frame}
  ...
  \begin{frame}
    ...
    \begin{itemize}
      \item<1-> For an axiom $\Gamma \mid \varphi \vdash \psi$ we have by definition that
        \[ \{\Gamma \mid \varphi \}^{(M)} \leq \{\Gamma \mid \psi \}^{(M)} \]
      \item<2-> Reflexivity $\Gamma \mid \varphi \vdash \varphi$ holds via the identity
  ...
  \begin{frame}
    ...
    \begin{itemize}
      ...
      \item<2-> $\Gamma \mid \varphi \vdash \top$
        By definition $\{\bar{x}:\bar{X} \mid \top\}^{(M)} = \bar{X}^{(M)}$
      \item<3-> Conjunction. We defined
  ...
  \begin{frame}
    \frametitle{Idea of the equivalence}
    In one direction, for two regular categories $\mathcal{E}, \mathcal{C}$ define
    \[ \mathfrak{M}_{M, \mathcal{C}} : \mathbf{RegCat}(\mathcal{E}, \mathcal{C}) \to \mathbf{Mod}(T, \mathcal{C})\]
    sends a regular functor $F : \mathcal{E} \to \mathcal{C}$ to a model $F(M)$.
    \pause \\
    Then take $\mathcal{E} = \mathcal{R}(T)$ and $M = \mathcal{U}$.
    \[ \mathfrak{M}_{\mathcal{C}} : \mathbf{RegCat}(\mathcal{R}(T), \mathcal{C}) \to \mathbf{Mod}(T, \mathcal{C})\]
  \end{frame}

  \begin{frame}
    \frametitle{Idea of the equivalence}
    Other direction define
    $$\mathfrak{F}_{\mathcal{C}} : \mathbf{Mod}(T, \mathcal{C}) \to \mathbf{RegCat}(\mathcal{R}(T), \mathcal{C})$$
    that maps a model $M$ in $\mathcal{C}$ to the functor
    \begin{align*}
      \mathfrak{F}_{\mathcal{C}}(M) : \mathcal{R}(T) &\to \mathcal{C}\\
      \{\bar{x}:\bar{X} \mid  p\} &\mapsto \{\bar{x}:\bar{X} \mid  p\}^{(M)} \\
      (\{\gamma\} : \{\bar{x} \mid  p\} \to \{\bar{y} \mid  q\})
                                                  &\mapsto (f : \{\bar{x} \mid  p\}^{(M)} \to \{\bar{y} \mid  q\}^{(M)}).
    \end{align*}
    \pause
    where $f$ is defined such that $\mathrm{graph}(f) = \{ \gamma \}$.
  \end{frame}
